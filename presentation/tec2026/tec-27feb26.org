#+STARTUP: showall
#+TITLE:     Is reelection an antidote to the incumbency curse?
...
* Motivation
** Motivation
*** Does winning office improve a party's \newline future electoral fortunes?
 \pause
# - Not in Mexico 1997--2010: \newline incumbent party's Pr(win) \alert{is 0.2 smaller} than runner-up's  (Lucardi--Rosas 2016)
# - Not in Mexico 1997--2010: \newline Pr(win|incumbent party) -- Pr(win|runner-up) $\approx$ --0.2  (Lucardi--Rosas 2016)
\bigskip
- Not in Mexico (Lucardi--Rosas 2016)
- Coalitions started from scratch at each cycle? \newline (Díaz Cayeros et al. 2016)
- ``Incumbency curse'' (also in Brazil, India, ...)
** Lucardi--Rosas
...
*** Study antedates last decade's \alert{reelection} reform
\rightarrow Could this change the curse into an incumbency advantage? \newline  (Erikson 1971, ...)
* The removal of single-term limits
** Electoral reform
*** Between 2018 and 2030:                                          :B_block:
    :PROPERTIES:
    :BEAMER_env: block
    :END:
# #+ATTR_BEAMER: :overlay +-
- Two-term limits for municipal officers \pause
- Staggered removal of single-term limits: \newline 23 states in 2018, 3 in 2019, 2 in 2021, 2 in 2024 \newline (and 2 non-reformers) \pause
- Many municipal presidents on the ballot 
...
#   \begin{tabular}{ccc}
#     Kick-off        & $N$ states  & States       \\ \hline
#     2018            &          23 & All others   \\
#     2019            &           3 & Ags BC Dgo   \\
#     2021            &           2 & CdMx Pue     \\
#     2024            &           2 & Nay Tla      \\
#     Non-reformers   &           2 & Hgo Ver      \\ \hline
#   \end{tabular} 
**** Table                                                  :B_ignoreheading:
    :PROPERTIES:
    :BEAMER_env: ignoreheading
...
#     :BEAMER_col: 0.55
#     :END:
# - Committee assignments 1k 
# - casework reports 1k
# *** Relative to rest, members who sought reelection
*** Relative to rest, members who sought reelection
- 2% likelier to mention pork in speeches
- spent 15% more in casework
- twice likelier to sit in pork committees
- but no change in party discipline \pause
*** Might this be different among \alert{executive} officers?
* Model
** Controlling incumbents on the ballot
*** Switching-regime model:                                         :B_block:
    :PROPERTIES:
    :BEAMER_env: block
    :END:
\footnotesize
...
** Three obstacles
1. 2018: reform coincided with party system collapse
2. Self-selection
3. Routine coalitions \pause
*** Willing and able? $ab$ confounded
|           <r> | <l>               | <l>            |
|               | static ambition   |                |
|               | 0                 | 1              |
|---------------+-------------------+----------------|
...
\pause
*** Three distinct subsets of observations
- pre-reform (and pre-AMLO): $ab_1$ 
- pre-kick-off: $ab_2$
- post-kick-off: $ab_3$ & $d_3$
# ***                                                         :B_ignoreheading:
#     :PROPERTIES:
#     :BEAMER_env: ignoreheading
#     :END:
...
# |-----------------+-----------+---------+---+-----------------+-----------+---------|
# | no              | $a_1$     | $b_1$   |   | no              | $a_2$     | $b_2$   |
# |                 |           | (empty) |   |                 |           | (empty) |
# |-----------------+-----------+---------+---+-----------------+-----------+---------|
# | yes             | $c_1$     | $d_1$   |   | yes             | $c_2$     | $d_2$   |
# |                 | (empty)   | (empty) |   |                 |           |         |

* Results
** Methodology
- Marginal ($\pm10\%$) municipal elections 1997--2025
- OLS and MCMC methods of estimation
- Use the different subsets
** OLS estimations
\centering
\includegraphics[width=.9\columnwidth]{../../plots/0-ols-table.png}
** Replication of Lucardi--Rosas 1997--2017
\centering
  \begin{tabular}{cc}
...
\centering
  ...
  \end{tabular}
** Tests: share of posterior draw conforming with curse hyps. 
...
***                                                         :B_ignoreheading:
    ...
    :BEAMER_env: ignoreheading
    :END:
- Before kick-off: incumbency curse permeated post-AMLO
- After: an incumbent running mitigated/inverted it
- Unlike state assemblies, reform effect is sizeable among municipal executives \newline $\rightarrow$ mayors with key sources of patronage in spoils system 
- Did counter-reformers kill an insurance policy against negative swings?
# *** Info
# [[https://github.com/emagar/mun-reel][\ExternalLink \url{https://github.com/emagar/mun-reel}]] 
