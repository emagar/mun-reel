...
\section{Data subsets for analysis}

\noindent Three obstacles to the study of single-term limit removal can be anticipated. One is associated with the collapse of the party system, one with competitors' self-selection, one with the ubiquity of electoral coalitions.

\subsection{The AMLO obstacle}
...
\noindent A critical election took place when the reform was set in motion. AMLO's landslide victory in the 2018 presidential election inaugurated a new party system. Morena, a splinter from the left, became rallying point, realigning voters from all three former major parties and independents too, thus achieving majority status that had eluded the former PAN--PRI--PRD troika. 

And concurrence raises a problem of overdetermination: if the probability of the incumbent party's reelection budged, is change attributable to the end of the reelection ban? To the party system? To both? If they pulled in opposite directions, the reform and Morena's sweep would cancel each other, partially or totally. 

I knock over this obstacle like Motolinia does, exploiting the staggered reform kick-off calendar as leverage. I proceed with analysis of different subsets of observations, summarized in Table \ref{T:data}.
